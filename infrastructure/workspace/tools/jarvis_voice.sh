...
if [ -f "$(dirname $(dirname $(dirname "$0")))/config/.env" ]; then
  source "$(dirname $(dirname $(dirname "$0")))/config/.env"
fi
...
# Build the command
CMD="python3 $(dirname $(dirname $(dirname "$0")))/tools/src/cli/auto_jarvis_voice.py"
...
if [ -z "$OPENAI_API_KEY" ]; then
  ENV_FILE="$(dirname $(dirname $(dirname "$0")))/config/.env"
