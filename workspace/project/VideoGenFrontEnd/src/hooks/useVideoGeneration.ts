...
import type { VideoGenerationResult, GenerationStatus } from '@/core/services/videos';
import type { Generation } from '@/types/project';
...
interface UseVideoGenerationOptions {
  ...
  onVideosUpdate?: (videos: Generation[]) => void;
}
            ...
            case 'completed': {
              ...
              try {
                ...
                const videosResponse =
                  await VideoGenerationService.getInstance().getProjectVideos(projectId);

                if (videosResponse.status === 'success' && videosResponse.data) {
                  const generatedVideo = videosResponse.data.find(
                    (video: Generation) => video.id === generationId,
                  );
                ...
                const errorMessage =
                  videosResponse.error?.message ?? 'Failed to refresh video list';
                ...
                const errorMessage =
                  error instanceof Error ? error.message : 'Failed to refresh video list';
