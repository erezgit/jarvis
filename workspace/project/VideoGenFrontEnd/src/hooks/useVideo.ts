...
import type { UseVideoReturn, VideoGenerationOptions } from '../types/hooks';

export function useVideo(): UseVideoReturn {
  const { data: projects, isLoading: projectsLoading, error: projectsError } = useProjects();
  const [uploadProgress, setUploadProgress] = useState(0);
  const [status, setStatus] = useState<UseVideoReturn['status']>('idle');
  ...
  const generateVideo = useCallback(
    async (options: VideoGenerationOptions) => {
      setStatus('uploading');
      setUploadProgress(0);
      ...
      try {
        ...
        const data = await new Promise<{ videoId: string }>((resolve, reject) => {
