import type { Video, VideoGeneration, VideoMetadata } from './types';
 ...
 */
export function mapGenerationToVideo(generation: VideoGeneration): Video {
  console.log(
    '[mapGenerationToVideo] Input:',
    JSON.stringify({
      id: generation.id,
 ...
 */
export function mapGenerationsToVideos(generations: VideoGeneration[]): Video[] {
 ...
 */
export function mapVideoToGeneration(video: Video): VideoGeneration {
  return {
    id: video.id,
...
export function mergeVideoMetadata(
  target: VideoMetadata | undefined,
  source: Partial<VideoMetadata> | undefined,
): VideoMetadata {
  if (!target && !source) {
    return {};
  }
